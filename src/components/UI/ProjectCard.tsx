...
export default function ProjectCard({ projectName, details, githubLink, techStack, tag }: ProjectCardProps) {
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 flex flex-col gap-3 hover:border-gray-300 transition-colors">
      ...
      <h3 className="font-serif text-lg font-semibold text-gray-900">{projectName}</h3>
      <p className="text-sm text-gray-500 leading-relaxed flex-1">{details}</p>
      <div className="flex flex-wrap gap-1.5">
        {techStack.map((tech) => (
          <span key={tech} className="font-mono text-xs bg-gray-50 text-gray-500 border border-gray-100 rounded px-2 py-0.5">
            {tech}
          </span>
        ))}
      </div>
